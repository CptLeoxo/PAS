...
barplot(ac,col=2:4,main="Airbagy v prodavanych vozech",ylab="Absolutni cetnosti",ylim=c(0,max(ac)+10))
x <- barplot(ac,plot=F)[,1]
text(x,ac,labels=ac,pos=3)
...
plot(x.val,ac.n,type="h",lwd=3,col="darkgreen",main="Frekvencni polygon",
     xlab="Pocet valcu",ylab="Absolutni cetnosti", ylim=c(0,max(ac.n)+10))
...
barplot(ac,col=2:8,main="Passanger",ylab="Absolutni cetnosti",ylim=c(0,max(ac)+10))
x <- barplot(ac,plot=F)[,1]
text(x,ac,labels=ac,pos=3)
popis<-paste(sort(unique(ab)),"(",round(rc*100,2),"%)")
pie(rc,lab=popis,col=2:8,main="Pass")
...
# zmena typu promenne kvuli popiskum na y-ove ose v grafu
plot(x.val,ac.n,type="h",lwd=3,col="darkgreen",main="Frekvencni polygon",
     xlab="Pass",ylab="Absolutni cetnosti", ylim=c(0,max(ac.n)+10))
x.val2<-min(x.val):max(x.val)
ac2<-rep(0,length(x.val2))
for(i in 1:length(x.val2)){
  for(j in 1:length(x.val)){if(x.val2[i]==x.val[j]){ac2[i]<-ac[j]}}
...
delka <- Cars93$Length
var(delka) # rozptyl -> rozpltyleni kolem mean() jak jsou daleko
sd(delka) # odchylka -> odmocnina rozptylu, ale ve stejnych jednotkach jako data
...
z.val<-scale(delka) # standarizace dat
  # Jak jsou velke? Co popisuji? K cemu se pouzivaji?

# Caharakteristiky tvaru rozdeleni
Skew(delka)    # mereni asymetrie rozdeleni dat, smer + velikost zkoseni
Kurt(delka)    # mereni kurtozy, ostre/ploche
...
hist(cena, col="skyblue", border="darkblue", main="Histogram cen automobilů",
     xlab="Cena automobilu", ylab="Počet automobilů")

boxplot(cena, main="Boxplot cen automobilů", ylab="Cena")
