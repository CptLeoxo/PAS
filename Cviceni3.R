...
fat <- Policie$fat
sfat <- table(fat)
hist(fat, main = "Fat Policie", xlab = "Fat Policie", ylab = "kg? idk", col = 2:9, ylim = c(0,max(hist(fat)$counts)*1.2))
...
# Nejprve musime urcit, kolik vrcholu (modu) promenna ma - z histogramu
...
jadro2 <- density(reakce)
hist(reakce)
lines(jadro2)
jadro2$x[which.max(jadro2$y)]
...
h2 <- 5
hist(tlak, plot=F)
cetnosti2 <- hist(tlak,plot=F)$counts
d01 <- cetnosti2[6]-cetnosti2[7]
d02 <- cetnosti2[6]-cetnosti2[5]
(modus2 <- A2 + h2*d01/(d01+d02))
